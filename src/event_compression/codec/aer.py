...
@codec(name="short_aer")
class ShortAER:
	"""
	Same as AER, shorter x,y fields
	Define encoder and decoder for AER format.
	...
	AER format represents each event as:
			value (1 byte), polarity (1 byte) i (1 bytes), j (1 bytes), t (4 bytes)
	...
	def decoder(cls, data) -> np.ndarray:
		...
		while True:
			try:
				buffer = take_next(data_it, 8)
			except:
				break

			i, j, t, value, polarity = struct.unpack('>2BI2B', buffer)
