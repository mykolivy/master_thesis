...
def make_event_preview(events, mode='red-blue', num_bins_to_show=-1):
	...
	if num_bins_to_show < 0:
		...
		sum_events = torch.sum(events[0, -num_bins_to_show:, :, :],
		                       dim=0).detach().cpu().numpy()

	if mode == 'red-blue':
		# Red-blue mode
		# positive events: blue, negative events: red
		event_preview = np.zeros((sum_events.shape[0], sum_events.shape[1], 3),
		                         dtype=np.uint8)
		b = event_preview[:, :, 0]
		r = event_preview[:, :, 2]
		b[sum_events > 0] = 255
...
class EventPreprocessor:
	...
	def __init__(self, options):
		...
		if options.hot_pixels_file:
			try:
				self.hot_pixel_locations = np.loadtxt(options.hot_pixels_file,
				                                      delimiter=',').astype(np.int)
...
class ImageWriter:
	...
	def __init__(self, options):
		...
		if self.output_folder:
			...
			print(f'Will write images to: {self.output_folder}')
			self.timestamps_file = open(join(self.output_folder, 'timestamps.txt'),
			                            'a')
	...
	def __call__(self, img, event_tensor_id, stamp=None, events=None):
		...
		if self.save_events and events is not None:
			event_preview = make_event_preview(events,
			                                   mode=self.event_display_mode,
			                                   num_bins_to_show=self.num_bins_to_show)
			cv2.imwrite(
			    join(self.event_previews_folder,
			         'events_{:010d}.png'.format(event_tensor_id)), event_preview)

		cv2.imwrite(
		    join(self.output_folder, 'frame_{:010d}.png'.format(self.frame_id)),
		    img)
		self.frame_id += 1
	...
	def __call__(self, img):
		if self.unsharp_mask_amount > 0:
			with Timer('Unsharp mask'):
				blurred = F.conv2d(img,
				                   self.gaussian_kernel,
				                   padding=self.gaussian_kernel_size // 2)
				img = (1 + self.unsharp_mask_amount
				       ) * img - self.unsharp_mask_amount * blurred
		return img
...
class ImageFilter:
	...
	def __call__(self, img):
		...
		if self.bilateral_filter_sigma:
			with Timer('Bilateral filter (sigma={:.2f})'.format(
			    self.bilateral_filter_sigma)):
				filtered_img = np.zeros_like(img)
				filtered_img = cv2.bilateralFilter(img, 5,
				                                   25.0 * self.bilateral_filter_sigma,
				                                   25.0 * self.bilateral_filter_sigma)
				img = filtered_img
...
class CropParameters:
	...
	def __init__(self, width, height, num_encoders):
		...
		self.pad = ReflectionPad2d((self.padding_left, self.padding_right,
...
def merge_channels_into_color_image(channels):
	...
	with Timer('Merge color channels'):
		...
		for channel in ['R', 'G', 'W', 'B']:
			channels[channel] = cv2.resize(channels[channel],
			                               dsize=None,
			                               fx=2,
			                               fy=2,
		...
		reconstruction_bgr = np.dstack([
		    channels['B'],
		    cv2.addWeighted(src1=channels['G'],
		                    alpha=0.5,
		                    src2=channels['W'],
		                    beta=0.5,
		                    gamma=0.0,
		                    dtype=cv2.CV_8U), channels['R']
		])
		...
		upsampled_img = upsample_color_image(reconstruction_grayscale,
		                                     reconstruction_bgr)
...
def events_to_voxel_grid(events, num_bins, width, height):
	...
	np.add.at(
	    voxel_grid, xs[valid_indices] + ys[valid_indices] * width +
...
def events_to_voxel_grid_pytorch(events, num_bins, width, height, device):
	...
	with torch.no_grad():
		...
		with DeviceTimer('Voxel grid voting'):
			voxel_grid = torch.zeros(num_bins,
			                         height,
			                         width,
			                         dtype=torch.float32,
			...
			events_torch[:, 0] = (num_bins - 1) * (events_torch[:, 0] -
			                                       first_stamp) / deltaT
			ts = events_torch[:, 0]
			...
			voxel_grid.index_add_(
			    ...
			    index=(xs[valid_indices] + ys[valid_indices] * width +
			           tis_long[valid_indices] * width * height).type(
			...
			voxel_grid.index_add_(
			    dim=0,
			    index=(xs[valid_indices] + ys[valid_indices] * width +
			           (tis_long[valid_indices] + 1) * width * height).type(
			               torch.cuda.LongTensor),
